...
impl Interpreter {
    ...
    fn execute_main(
        ...
        let mut current_frame = Frame::new(
            ...
            ty_args,
            locals,
            self.runtime_config.paranoid_type_checks,
        )
        ...
        loop {
            ...
            match exit_code {
                ...
                ExitCode::Call(fh_idx) => {
                    ...
                    let frame = self
                        .make_call_frame(func, vec![], self.runtime_config.paranoid_type_checks)
                        .map_err(|e| self.set_location(e))
                        .map_err(|err| self.maybe_core_dump(err, &current_frame))?;
                ...
                ExitCode::CallGeneric(idx) => {
                    ...
                    let frame = self
                        .make_call_frame(func, ty_args, self.runtime_config.paranoid_type_checks)
                        .map_err(|e| self.set_location(e))
                        .map_err(|err| self.maybe_core_dump(err, &current_frame))?;
                    self.call_stack.push(current_frame).map_err(|frame| {
                        let err = PartialVMError::new(StatusCode::CALL_STACK_OVERFLOW);
    ...
    fn make_call_frame(
        ...
        Frame::new(func, ty_args, locals, paranoid)
    }
    ...
    fn call_native_impl(
        ...
        if self.runtime_config.paranoid_type_checks {
            ...
            for i in 0..expected_args {
                let expected_ty = if is_generic {
                    ...
                    function.parameter_types()[expected_args - i - 1].clone()
                };
                let ty = self.operand_stack.pop_ty()?;
        ...
        if self.runtime_config.paranoid_type_checks {
            ...
            for ty in function.return_types() {
                self.operand_stack.push_ty(if is_generic {
                    ty.subst(&ty_args)?
                } else {
                    ty.clone()
                })?;
    ...
    fn simple_instruction_type_stack_transition(
        ...
        match instruction {
            ...
            Bytecode::Pack(idx) => {
                ...
                let field_abilities = if ability.has_key() {
                    ability
                        .remove(Ability::Key)
                ...
                {
                    check_ability(field_abilities.is_subset(resolver.loader().abilities(&ty)?))?;
                    ty.check_eq(expected_ty)?;
                }
            ...
            Bytecode::WriteRef => {
                ...
                let val_ty = interpreter.operand_stack.pop_ty()?;
                match ref_ty {
                    Type::MutableReference(inner) if *inner == val_ty => {
                    ...
                    _ => {
                        ...
                        .with_message("WriteRef expecting a value of mutable type".to_string()))
                    }
                }
            }
            Bytecode::CastU8 => {
    ...
    fn execute_code_impl(
        ...
        loop {
            for instruction in &code[self.pc as usize..] {
                ...
                if interpreter.runtime_config.paranoid_type_checks {
