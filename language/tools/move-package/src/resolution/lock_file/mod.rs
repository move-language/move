...
impl LockFile {
    ...
    pub fn new(package_root: &Path) -> Result<LockFile> {
        ...
        write_lock_file_prologue(&mut lock).context("Initializing lock file")?;
...
fn write_lock_file_prologue(file: &mut NamedTempFile) -> io::Result<()> {
    writeln!(
        file,
    ...
    writeln!(file)?;
    writeln!(file, "[move]")?;
    writeln!(file, "version = {}", schema::VERSION)?;
